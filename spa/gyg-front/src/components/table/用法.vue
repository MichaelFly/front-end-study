    ...
    methods: {
      initTable (dataList, pageNo, pageSize, total) {
        ...
        vm.tableOptions = {
          ...
          isOperateflag:self.data.operateflag,//用户操作权限 0-不可操作；1可操作
          operation:{
            fixed: "rihgt", //操作列固定，如果不需要固定，此项不用写
            width: 150, //操作列宽度，如果不需要，此项不用写
            items: [{
              xtype: "link", // linkButton
              handleType:"0", //按钮性质：2-查看类按钮（不涉及操作的）；0-操作类（例：删除，编辑，开始，停止，合格等）
              ...
              xtype: "button",
              handleType:"2", //按钮性质：2-查看类按钮（不涉及操作的）；0-操作类（例：删除，编辑，开始，停止，合格等）
