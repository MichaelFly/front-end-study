  ...
  <el-main class="wrap">
    <div class="breadWrap">
      <div class="breadContent clearfix">
        ...
        <el-breadcrumb class="breadRight" separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item :to="{ path: '/companyRes/employeeMore' }">就业招工</el-breadcrumb-item>
    ...
    <div class="content">
      <div class="job-name clearfix">
        <p>
          <span v-if="jobDetails.publishTime">发布时间：{{jobDetails.publishTime}}</span>
          <span v-if="!jobDetails.publishTime">发布时间：{{nowTime |formatDate}}</span>
          <span v-if="jobDetails.visitCount">浏览量：{{jobDetails.visitCount}}</span>
          <span v-if="!jobDetails.visitCount">浏览量：0</span>
        ...
        <p>
          ...
          <span>|</span>
          <span>年龄要求 : {{jobDetails.age}}岁</span>
          <span>|</span>
      ...
      <div class="other-jobs clearfix" v-if="getMore">
        ...
        <div v-for="(item,index) in jobMore" :key="index" v-if="index < 7 && item.id !=jobDetails.id" @click="getJobDetail(item.id)">
          <p>{{item.title}}</p>
          <p>
            <span class="salary">{{item.salary}}</span>
            <span  class="yuan">{{item.yuan}}</span>
            <span>{{item.publishTime |cutTime}} 发布</span>
          </p>
          <p>
