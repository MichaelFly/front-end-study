    ...
    <div class="breadWrap">
      <div class="breadContent clearfix">
        ...
        <el-breadcrumb class="breadRight" separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item>违法事件</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
    </div>
    <div class="content">
      ...
      <div class="list clearfix">
        <div v-for="(item,index) in list" :key="index" @click="rowClick(item.id)">
          <i class="dot"></i>
          <span v-if="item.isTop" class="isTop">[置顶]</span>
          <p>{{item.title}}</p>
  ...
  export default {
    ...
    methods:{
      ...
      rowClick(row,event,column){
        let vm = this;
        console.log(row.id);
        window.open(this.$router.resolve({path:"/illegalAct/lawIncidentDetail",query:{id:row}}).href, '_blank');
        // vm.$router.push({path:"/illegalAct/lawIncidentDetail",query:{id:row}})
      },
      path(val){
        let vm = this;
        switch (val) {
